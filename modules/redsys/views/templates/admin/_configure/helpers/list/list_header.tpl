...
{block name="startForm"}
	<form method="post" action="{$action|escape:'html':'UTF-8'}" class="col form-horizontal clearfix" id="form-{$list_id}">
{/block}
		...
		{if version_compare($smarty.const._PS_VERSION_, '1.7.6', '>=')}
			<div class="card-header">
				<h3 class="card-header-title">
					{if is_array($title)}{$title|end|escape:'html':'UTF-8'}{else}{$title|escape:'html':'UTF-8'}{/if}
				</h3>
			</div>
		{else}
